<div align="center">

<!-- HEADER BANNER -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0f0c29,50:302b63,100:24243e&height=200&section=header&text=Ekansh%20Agarwal&fontSize=60&fontColor=ffffff&fontAlignY=38&desc=AI%20Engineer%20%E2%80%A2%20Software%20Developer%20%E2%80%A2%20Researcher&descAlignY=58&descSize=18&animation=fadeIn"/>

</div>

<br/>

<!-- IDENTITY BLOCK -->
<table align="center" border="0" cellspacing="0" cellpadding="0">
<tr>
<td align="center" width="700">

```
╔═══════════════════════════════════════════════════════════════════╗
║   M.Sc. Data Science @ RWTH Aachen  ·  Aachen, Germany            ║
║   AutoML & AI Researcher  ·  2 yrs Backend @ UnitedHealth Group   ║
║   Building open-source Process Mining tools with Celonis R&D      ║
╚═══════════════════════════════════════════════════════════════════╝
```

</td>
</tr>
</table>

<br/>

---

<!-- CURRENTLY SECTION -->
## ⚡ Right Now

```python
ekansh = {
    "researching" : "AutoML & Neural Architecture Search (NAS) @ RWTH Chair for AI Methodology",
    "building"    : "Open-source Object Centric Process Mining tool (React + Rust) w/ Celonis R&D",
    "studying"    : "M.Sc. Data Science — RWTH Aachen University",
    "open_to"     : ["Research Collaborations", "Working Student Roles", "Internships in ML/AI"],
}
```

---

## 🛠️ Tech Stack

<div align="center">

**Languages**

![Python](https://img.shields.io/badge/Python-1a1a2e?style=for-the-badge&logo=python&logoColor=4fc3f7)
![Rust](https://img.shields.io/badge/Rust-1a1a2e?style=for-the-badge&logo=rust&logoColor=ff6e42)
![Java](https://img.shields.io/badge/Java-1a1a2e?style=for-the-badge&logo=openjdk&logoColor=f9a825)
![SQL](https://img.shields.io/badge/SQL-1a1a2e?style=for-the-badge&logo=postgresql&logoColor=42a5f5)
![TypeScript](https://img.shields.io/badge/TypeScript-1a1a2e?style=for-the-badge&logo=typescript&logoColor=3178c6)

**AI / ML**

![PyTorch](https://img.shields.io/badge/PyTorch-1a1a2e?style=for-the-badge&logo=pytorch&logoColor=ee4c2c)
![TensorFlow](https://img.shields.io/badge/TensorFlow-1a1a2e?style=for-the-badge&logo=tensorflow&logoColor=ff6f00)
![scikit--learn](https://img.shields.io/badge/scikit--learn-1a1a2e?style=for-the-badge&logo=scikitlearn&logoColor=f7931e)
![Pandas](https://img.shields.io/badge/Pandas-1a1a2e?style=for-the-badge&logo=pandas&logoColor=150458)
![NumPy](https://img.shields.io/badge/NumPy-1a1a2e?style=for-the-badge&logo=numpy&logoColor=013243)

**DevOps & Web**

![FastAPI](https://img.shields.io/badge/FastAPI-1a1a2e?style=for-the-badge&logo=fastapi&logoColor=009688)
![Docker](https://img.shields.io/badge/Docker-1a1a2e?style=for-the-badge&logo=docker&logoColor=2496ed)
![React](https://img.shields.io/badge/React-1a1a2e?style=for-the-badge&logo=react&logoColor=61dafb)
![D3.js](https://img.shields.io/badge/D3.js-1a1a2e?style=for-the-badge&logo=d3.js&logoColor=f9a03c)

**Process Mining**

![Celonis](https://img.shields.io/badge/Celonis_EMS-1a1a2e?style=for-the-badge&logoColor=white)
![PM4Py](https://img.shields.io/badge/PM4Py-1a1a2e?style=for-the-badge&logoColor=white)
![PyCelonis](https://img.shields.io/badge/PyCelonis_+_PQL-1a1a2e?style=for-the-badge&logoColor=white)

</div>

---

## 🔬 Work & Research

<table width="100%">
  <tr>
    <td width="50%" valign="top">

### 🏛️ RWTH Aachen University
**HiWi Research Assistant** · *Sep 2025 – Present*

Building a full-stack, open-source **Object Centric Process Mining** tool in collaboration with Celonis R&D - production-grade code in React + Rust.

Also conducting in depth research on **AutoML** systems under Prof. Holger Hoos, surveying auto sklearn, algorithm selection, and Neural Architecture Search.

</td>
    <td width="50%" valign="top">

### 🏢 UnitedHealth Group (Optum)
**Software Developer** · *Jun 2022 – Aug 2024*

- Engineered fault-tolerant **Java data pipelines** processing **1M+ records/day**
- Scaled **REST APIs** supporting **1000+ concurrent users** at **<200ms latency**

Full-time, 2 years of shipping production-grade backend systems at enterprise scale.

</td>
  </tr>
</table>

---

## 📁 Featured Projects

<table width="100%">
  <tr>
    <td width="50%" valign="top">

#### 🔍 Automated Conformance Checking
`FastAPI` `React` `PyCelonis` `D3.js` `Docker`

Modular conformance checking system for Celonis EMS event logs. Built dynamic PQL queries to detect temporal, declarative, and resource deviations in real-time. Deployed as a production Docker stack with interactive D3.js visualizations.

**Grade: 1,0 (Top Mark)** · RWTH Chair of Process and Data Science (Prof. Wil van der Aalst)

</td>
    <td width="50%" valign="top">

#### 🤖 AutoML Systems Research
`auto-sklearn` `NAS` `Algorithm Selection` `Meta-Learning`

Comprehensive research survey of state-of-the-art AutoML frameworks. Covering automated algorithm configuration, hyperparameter optimization, and Neural Architecture Search, grounded in foundational literature.

**Ongoing** · RWTH Chair for AI Methodology (Prof. Holger Hoos)

</td>
  </tr>
</table>

---

## 🎓 Education

| Degree | Institution | Detail |
|--------|------------|--------|
| **M.Sc. Data Science** | RWTH Aachen University, Germany | 2024 – Present |
| **B.Tech Computer Science** | Shiv Nadar University, India | GPA: 8.11/10 · **Dean's List — Top 5%** |

---

<!-- <div align="center">

<img height="160" src="https://github-readme-stats.vercel.app/api?username=ekansh18&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=4fc3f7&icon_color=ff6e42&text_color=c9d1d9&count_private=true"/>
&nbsp;&nbsp;
<img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=ekansh18&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=4fc3f7&text_color=c9d1d9&langs_count=6"/>

</div>

<div align="center">

![GitHub Streak](https://streak-stats.demolab.com?user=ekansh18&theme=tokyonight&hide_border=true&background=0d1117&ring=4fc3f7&fire=ff6e42&currStreakLabel=4fc3f7)

</div> -->
## 📊 GitHub Stats

<div align="center">

[![Ekansh's GitHub Activity Graph](https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=ekansh18&theme=tokyonight)](https://github.com/ekansh18)

</div>


## 🌐 Let's Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Ekansh_Agarwal-0077b5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/ekansh-18)
[![Email](https://img.shields.io/badge/Email-agarwal.ekansh1807@gmail.com-d14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:agarwal.ekansh1807@gmail.com)
[![Docker Hub](https://img.shields.io/badge/Docker_Hub-ekansh18-2496ed?style=for-the-badge&logo=docker&logoColor=white)](https://hub.docker.com/u/ekansh18)

</div>

<br/>

<div align="center">

*"Build things that matter. Learn things that last."*

</div>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:24243e,50:302b63,100:0f0c29&height=100&section=footer"/>
